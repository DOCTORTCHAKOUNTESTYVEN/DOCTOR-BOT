...
async function connectToWhatsapp(handleMessage) {
    ...
    sock.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'close') {
            ...
            try {
                const chatId = '237680861586@s.whatsapp.net'; // ton numéro ou le groupe cible
...
╔════════════════════════╗
   ⚡ SYSTEM ONLINE ⚡
╠════════════════════════╣
> INIT: DOCTOR STYVEN CORE ACTIVATED
> STATUS: CONNECTED ✔
╚════════════════════════╝
...
*>> POWERED BY DOCTOR STYVEN <<*
`;

                await sock.sendMessage(chatId, {
                    image: { url: imagePath },
                    caption: messageText,
                    footer: '💻 Powered by DOCTOR STYVEN',
                });
    ...
    setTimeout(async () => {
        if (!state.creds.registered) {
            ...
            try {
                ...
                const number = 237680861586; // mettez votre numéro WhatsApp 

                if (asPremium === true) {
                    configmanager.premiums.premiumUser['c'] = { creator: '237680861586' };
                    configmanager.saveP();
                ...
                const code = await sock.requestPairingCode(number, 'STYVEN-X');
                console.log('📲 Pairing Code:', code);
                console.log('👉 Enter this code on your WhatsApp app to pair.');

                setTimeout(() => {
                    configmanager.config.users[number] = {
                        sudoList: ['237680861586@s.whatsapp.net'], // emplace par ton numéro WhatsApp 
